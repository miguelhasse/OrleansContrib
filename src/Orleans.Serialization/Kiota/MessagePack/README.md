...
An Orleans serialization codec for Microsoft Kiota `IParsable` models using MessagePack.

## Overview

Kiota-generated clients expose models through `IParsable`, which Orleans does not serialize out of the box. This package registers a generalized Orleans codec (`KiotaMessagePackCodec`) that:

- accepts concrete `IParsable` types
- serializes them with Kiota's MessagePack serializer
...
| File | Description |
|---|---|
| `KiotaMessagePackCodec` | Generalized Orleans codec and copier for all supported `IParsable` types. Registered with alias `kiota-msgpack`. |
| `KiotaMessagePackOptions` | Options type with the `Compression` flag. |
...
- .NET 10+
- `Microsoft.Kiota.Serialization.MessagePack`
