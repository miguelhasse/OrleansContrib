...
public sealed class KiotaJsonCodec(IOptions<KiotaJsonCodecOptions>? options) : IGeneralizedCodec, IGeneralizedCopier, ITypeFilter
{
    ...
    /// </summary>
    public const string WellKnownAlias = "kiota-json";
    ...
    public void WriteField<TBufferWriter>(ref Writer<TBufferWriter> writer, uint fieldIdDelta, Type expectedType, object value) where TBufferWriter : IBufferWriter<byte>
    {
        ...
        try
        {
            using (var jsonWriter = new BufferJsonSerializationWriter(bufferWriter, KiotaJsonSerializationContext.Default))
                jsonWriter.WriteObjectValue(null, (IParsable?)value);
                    ...
                    try
                    {
                        ...
                        if (tempBuffer.Length > 0)
                        {
                            if (compressed)
                            {
                                ...
                                jsonDocument = JsonDocument.Parse(decompressed.Memory);
                            }
                            else
                            {
                                jsonDocument = JsonDocument.Parse(tempBuffer.ToArray());
                            }
                        }
                        ...
                        {
                            var parseNode = new JsonParseNode(jsonDocument.RootElement.Clone(), KiotaJsonSerializationContext.Default);
    ...
    public object? DeepCopy(object? input, CopyContext context)
    {
        ...
        {
            using (var jsonWriter = new BufferJsonSerializationWriter(bufferWriter, KiotaJsonSerializationContext.Default))
                jsonWriter.WriteObjectValue(null, parsable);

            using var jsonDocument = JsonDocument.Parse(bufferWriter.Value.ToArray());
            var parseNode = new JsonParseNode(jsonDocument.RootElement.Clone(), KiotaJsonSerializationContext.Default);
            result = parseNode.GetObjectValue(ParsableFactoryHelper.Create(type))
